...
import { AfterViewChecked, AfterViewInit, Component, ContentChildren, ElementRef, EventEmitter, HostBinding, HostListener, Inject, Input, OnChanges, OnDestroy, OnInit, Output, QueryList, Renderer2, ViewChild } from '@angular/core';
import { CarouselItemDirective } from './carousel-item.directive';

@Component({
  selector: 'awaresomeco-carousel',
  templateUrl: "./lib-awaresomeco-carousel.html",
  styleUrls: ['./lib-awaresomeco-carousel.css']
})
export class AwaresomecoCarouselComponent implements OnDestroy, AfterViewInit, OnChanges, AfterViewChecked  {
  ...
  @ContentChildren(CarouselItemDirective, { descendants: true }) _children!: QueryList<CarouselItemDirective>;
  ...
  onMouseDownHandler(event: MouseEvent) {
    const dragScrollItem: CarouselItemDirective | null = this.locateDragScrollItem(event.target as Element);
  ...
  */
  ...
  private locateDragScrollItem(element: Element): CarouselItemDirective | null {
    let item: CarouselItemDirective | null = null;
    const childrenArr = this._children.toArray();
    for (let i = 0; i < childrenArr.length; i++) {
  ...
  private maximumIndex(containerWidth: number, childrenElements: CarouselItemDirective[]): number {
    let count = 0;
    let childrenWidth = 0;
    for (let i = 0; i <= childrenElements.length; i++) {
      // last N element
      const CarouselItemDirective: CarouselItemDirective = childrenElements[childrenElements.length - 1 - i];
      if (!CarouselItemDirective) {
        ...
        if (itemWidth === 0 && nativeElement.firstElementChild) {
          itemWidth = CarouselItemDirective._elementRef.nativeElement.firstElementChild.clientWidth;
        }
